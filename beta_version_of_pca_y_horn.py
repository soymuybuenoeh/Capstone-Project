# -*- coding: utf-8 -*-
"""Beta Version of PCA Y HORN.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/197u1ksZq402_WraiIEyMJwL8a15Mg9zH
...
# Lista de países
countries = [
    "ghana", "belize", "slovenia", "slovakia"
]
...
def get_bert_embeddings(paragraphs, word):
    ...
    for paragraph in paragraphs:
        ...
        last_hidden_state = outputs.last_hidden_state.cpu()  # Volver a CPU para numpy
        word_ids = tokenizer.encode(word, add_special_tokens=False)

        word_idx = (inputs["input_ids"].cpu() == word_ids[0]).nonzero(as_tuple=False)
        paragraph_embeddings = [last_hidden_state[0][idx[1]].numpy() for idx in word_idx]
        all_embeddings.extend(paragraph_embeddings)
...
for country in countries:
    ...
    try:
        ...
        pca = PCA(n_components=n_components)
        subspace = pca.fit_transform(X).T
        print(f"📏 Subespacio reducido: {subspace.shape}")
    except Exception as e:
        print(f"❌ Error procesando {country.title()}: {e}")
...
# Lista de países
countries = [ "india", "united kingdom", "singapore", "sweden", "norway",
    "portugal", "ecuador", "peru", "burkina faso", "french guiana", "mongolia", "bulgaria", "oman"
]
